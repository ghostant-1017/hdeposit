...
use lighthouse_types::{Hash256, PublicKey, WithdrawalCredentials, PublicKeyBytes};

pub async fn upsert_validators(
    client: &Client,
    ...
    for validator in validators {
        ...
        client.execute(sql, &[&index,&pubkey, &wc, &amount, &data]).await?;
    }
    Ok(())
}
...
pub async fn upsert_validators_by_logs(client: &Client, logs: &Vec<DepositEventFilter>) -> Result<()> {
    ...
    for log in logs {
        ...
        client.execute(sql, &[&index, &serde_json::to_string(&pubey)?,&serde_json::to_string(&wc)?, &amount]).await?;
    }
    Ok(())
}