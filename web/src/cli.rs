...
use clap::Parser;
use eth2::{BeaconNodeHttpClient, SensitiveUrl, Timeouts, Url, types::{ChainSpec, Config, MainnetEthSpec, ConfigAndPreset}};
...
impl Cli {
    pub async fn exec(self) -> anyhow::Result<()> {
        ...
        let config_and_preset: ConfigAndPreset = beacon.get_config_spec().await.expect("get config from beacon").data;
        let spec = ChainSpec::from_config::<MainnetEthSpec>(config_and_preset.config()).ok_or(anyhow::anyhow!("from config"))?;
