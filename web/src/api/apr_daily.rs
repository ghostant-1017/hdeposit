...
use storage::models::{
    select_max_epoch, select_range_cl_rewards, select_range_el_rewards,
    select_range_validators_count,
};
...
pub async fn get_apr_daily(
    ...
    let cl_rewards: HashMap<Epoch, u64> = select_range_cl_rewards(tx.client(), from, to)
        ...
        .into_iter()
        .collect();
    let el_rewards: HashMap<Epoch, u64> = select_range_el_rewards(tx.client(), from, to)
        .await?
        .into_iter()
        .collect();
    drop(tx);
