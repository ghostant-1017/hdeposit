...
pub async fn get_daily_rewards_7days(
    ...
    let today = now.date_naive().and_hms_opt(0, 0, 0).ok_or(anyhow!("time error"))?;
    // Caculate the range [start, end)
    let end= server.clock.slot_of(Duration::from_secs(today.and_utc().timestamp() as u64)).ok_or(anyhow!("slot error"))?;
    let start = end - SLOTS_PER_DAY * 7;
    let batch = select_withdrawals_by_wc_range(&db, wc, start.as_u64() as i64, end.as_u64() as i64).await?;
    ...
    for w in batch {
        ...
        result.entry(day).and_modify(|val| *val += w.withdrawal.amount);
    }
...
fn slot_to_day(clock: &SystemTimeSlotClock, slot: Slot) -> anyhow::Result<NaiveDateTime> {
    ...
    let day = NaiveDateTime::from_timestamp_opt(d.as_secs() as i64, 0).ok_or(anyhow!("from timestamp opt"))?;
    let day = day.date().and_hms_opt(0, 0, 0).ok_or(anyhow!("and hms opt"))?;
    Ok(day)
}
