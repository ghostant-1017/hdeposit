...
use eth2::lighthouse_vc::{http_client::ValidatorClientHttpClient, types::UpdateFeeRecipientRequest};
use storage::{db::PgPool, models::{query_all_events, query_deposit_data_by_evt_pk}};
use anyhow::Result;
    ...
    async fn do_update(&self) -> Result<()> {
        ...
        for evt in evts {
            if !evt.log.create_el_fee {
                warn!("[FeeManager]Found `PreDepositEvent` create_el_fee is false, pk: {}.", evt.pk);
                continue;
            };
            ...
            for pubkey in pubkeys {
                ...
                info!("[FeeManager]Trying to set pubkey: {}, fee recipient: {}", pubkey, req.ethaddress);
                if let Err(err) = self.validator_client.post_fee_recipient(&pubkey, &req).await {
